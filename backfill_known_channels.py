...
def _publish_channels(
    api_url: str, bucket: str, campaign: str, channels: List[Dict[str, str]]
) -> None:
    """POST vers /buckets/{bucket}/channels (scope bucket, pas clientId)."""
    url = f"{api_url.rstrip('/')}/buckets/{bucket}/channels"
    resp = requests.post(
        url,
